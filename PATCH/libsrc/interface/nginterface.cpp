...
int Ng_GetElement_Faces (Mesh* mesh, int elnr, int * faces, int * orient)
{
	const MeshTopology & topology = mesh->GetTopology();
	if (mesh->GetDimension() == 3)
	return topology.GetElementFaces (elnr, faces, orient);
...
void writeOF(Ng_Mesh *mesh__, const char* filename) {
	Mesh *mesh_ = (Mesh*) mesh__;
	WriteUserFormat("OpenFOAM 1.5+ Format", *mesh_, *ng_geometry, filename);
}
